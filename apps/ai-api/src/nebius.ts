...
export class NebiusTrailGenerator {
  ...
  async generate(topic: string) {
    const response = await this.request(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Create exactly five scenes and one four-option recall question about: ${topic}. Return {"scenes":[{"place":"","emoji":"","title":"","story":"","anchor":""}],"quiz":{"prompt":"","options":["","","",""],"correct":0,"explanation":""}}. The correct index must match the correct option.` },
        ],
      }),
    });
    ...
    return finishTrail(topic, parsed.data);
  }
}
