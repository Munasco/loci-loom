...
export function isMemoryTrail(value: unknown): value is MemoryTrail {
  ...
  if (!trail.scenes.every((scene) => isText(scene.place) && (scene.palacePlace === undefined || isText(scene.palacePlace)) && isText(scene.emoji) && isText(scene.title) && isText(scene.story) && isText(scene.anchor) && Array.isArray(scene.gradient) && scene.gradient.length === 2 && scene.gradient.every(isText))) return false;
