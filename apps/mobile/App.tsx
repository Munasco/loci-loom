...
import { PalaceBuilder } from './src/components/PalaceBuilder';
...
export default function App() {
  ...
  if (screen === 'map') return <Shell><TopBar title="Build your memory palace" onBack={() => go('home')} /><ScrollView contentContainerStyle={styles.mapPage}>
    <Text style={styles.mapEyebrow}>{trail.sourceProvided ? 'IDEAS WOVEN FROM YOUR NOTES' : 'MAKE THE ROOM YOURS'}</Text><Text style={styles.mapTitle}>{trail.topic}</Text><Text style={styles.mapIntro}>Place each memory somewhere unmistakable. Choosing the location yourself makes the room easier to reconstruct later.</Text>
    <PalaceBuilder trail={trail} onComplete={places => { const placed = { ...trail, scenes: trail.scenes.map((item, index) => ({ ...item, palacePlace: places[index] })) }; setTrail(placed); setSavedTrails(items => [placed, ...items.filter(item => item.id !== placed.id)]); void repository.save(placed); setScene(0); go('trail'); }} />
  </ScrollView></Shell>;
